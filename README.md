...
将微信聊天历史转化为有证据支持的叙事弧。双层索引架构，支持自然语言查询关系动态变化。
 ...
 想了解系统设计的整体思路？可阅读这篇博文：[narrarc 系统设计详解](https://linux.do/t/topic/1672385)

## 截图与核心功能

### 数据接入与索引构建 (Init Flow)
![数据接入与索引构建演示](./pic/initchat.gif)
导入聊天记录后，系统自动按时间切分 Burst、抽取 TopicNode，完成双层索引构建。

### 自然语言叙事查询演示 (Query Flow)
![自然语言叙事查询演示](./pic/question.gif)
输入自然语言问题，系统实时串联话题脉络，并附上原始聊天记录作为证据支撑。
